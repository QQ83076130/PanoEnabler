...
- (void)addBtn
{
	UIBarButtonItem *hideKBBtn = [[UIBarButtonItem alloc] initWithTitle:@"Hide KB" style:UIBarButtonItemStyleBordered target:self action:@selector(hideKeyboard)];
...
{	
	...
	if (self) {
		...
		[self.label setText:@"PanoMod"];
		[self.label setBackgroundColor:[UIColor clearColor]];
		[self.label setFont:[UIFont fontWithName:@"HelveticaNeue" size:60]];
		[self.label setTextAlignment:NSTextAlignmentCenter];
		[self.label setAutoresizingMask:2];
		[self.label setTextColor:[UIColor colorWithRed:.4 green:.4 blue:.49 alpha:1]];
...
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section
{
	switch (section) {
		...
		case 20: return @"About \"Hide KB\" button at Top-right";
	}
	return nil;
}
...
{
   	CGSize constraint = CGSizeMake([tableView frame].size.width - (CELL_CONTENT_MARGIN * 2), MAXFLOAT);
  	return 30 + [[[[self tableView:tableView cellForRowAtIndexPath:indexPath] detailTextLabel] text] sizeWithFont:[UIFont systemFontOfSize:(kFontSize + 0.5)] constrainedToSize:constraint lineBreakMode:NSLineBreakByWordWrapping].height;
}
